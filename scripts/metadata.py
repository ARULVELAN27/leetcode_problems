...
from helpers import is_problem_folder, extract_problem_id, extract_slug
...
def main():
    ...
    for folder in folders:
        ...
        # Use problem_index if available, otherwise fall back to folder slug
        if problem_id in problem_index:
            slug = problem_index[problem_id]["slug"]
        else:
            slug = extract_slug(folder)
            print(f"⚠ Problem {problem_id} not in index. Using folder slug: {slug}")
        ...
        primary_topic = topic_map.get(primary_tag, primary_tag)
